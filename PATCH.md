...
### Added
- 랭킹 알림 기간 선택 UI를 추가했습니다.
  - 설정 화면에서 랭킹 알림을 켜면 `일간 / 주간 / 월간` 중 원하는 기간을 고를 수 있습니다.
  - 기간 선택 영역은 토글 아래에서 부드럽게 펼쳐지고 닫히도록 애니메이션을 적용했습니다.

### Changed
- 랭킹 알림 범위를 `일간 전체` 기준에서 `기간별 난이도 랭킹` 기준으로 확장했습니다.
  - 이제 사용자가 선택한 기간의 `쉬움 / 보통 / 어려움` 랭킹 알림만 표시됩니다.
  - 알림 헤더 문구도 `일간 쉬움 랭킹`, `주간 보통 랭킹`, `월간 어려움 랭킹`처럼 실제 버킷에 맞게 바뀝니다.
- 게임 클리어 직후 받는 랭킹 알림 응답이 단일 항목에서 목록 형태로 바뀌었습니다.
  - 프론트는 이 목록 중 현재 설정한 기간과 일치하는 알림만 즉시 큐에 넣어 재생합니다.
- 랭킹 알림 관련 번역을 `ko / en / ja / zh` 4개 언어 기준으로 정리했습니다.
  - 기간별 난이도 버킷 라벨과 신규 진입 / 추월 헤더 문구가 모두 각 언어에서 자연스럽게 보이도록 다듬었습니다.
- SettingsView 버전 표기를 `2.16.3`으로 갱신했습니다.

### Notes
- Web 버전을 `2.16.2`에서 `2.16.3`으로 올렸습니다.
...
### Changed
- 일간 랭킹 알림 헤더의 강조 표현을 다듬었습니다.
  - 추월 알림에서 추월한 플레이어와 제쳐진 플레이어의 이름을 더 구분해서 볼 수 있습니다.
  - 달성한 랭킹 숫자는 더 눈에 띄도록 강조했습니다.
- 랭킹 카드 펼침 UI를 더 정돈했습니다.
  - 접힌 상태의 랭킹 카드 내용이 세로 가운데에 더 자연스럽게 정렬됩니다.

### Fixed
- 추월 알림에서 등장하는 랭킹 카드의 불꽃 스파크가 보이지 않던 문제를 수정했습니다.
  - 이제 추월 카드가 나타나는 타이밍에 카드 뒤쪽에서 스파크가 함께 재생됩니다.

### Notes
- Web 버전을 `2.16.1`에서 `2.16.2`로 올렸습니다.
...
### Changed
- 설정 화면 알림 토글 스위치의 라이트 모드 가시성을 높였습니다.
  - 비활성화 상태 트랙, 테두리, thumb 그림자 대비를 높여 흰 배경에서도 토글 경계가 더 또렷하게 보이도록 조정했습니다.
  - 활성화 상태 트랙과 포커스 링 대비도 함께 보정해 켜짐 여부와 포커스 상태를 더 빠르게 구분할 수 있도록 정리했습니다.
- 알림 토글 스위치 thumb 이동 범위를 미세 조정했습니다.
  - 비활성화 상태에서는 thumb가 더 왼쪽 끝에 붙어 보이도록 시작 위치를 당겼습니다.
  - 활성화 상태에서는 thumb가 조금 더 오른쪽까지 이동하도록 끝 위치를 늘려 상태 전환이 더 분명하게 보이도록 맞췄습니다.
- SettingsView 버전 표기를 `2.16.1`로 갱신했습니다.

### Notes
- Web 버전을 `2.16.0`에서 `2.16.1`로 올렸습니다.
